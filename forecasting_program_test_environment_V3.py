...
def plot_model_figures(model_results, cleaned_dataframe : pd.DataFrame, alpha_level : int, hyperparameter_combo : list):
    ...
    return decomposition_fig, model_results.plot_diagnostics(figsize=(18, 8)), forecast_overlay_fig
...
def forecast_results(cleaned_dataframe : pd.DataFrame, alpha_level : int, hyperparameter_combo : list, prediction_start : str, prediction_end : str):
    ...
    st.markdown(f"<h3 style='text-align: center'>{merged_table_title}</h2>", unsafe_allow_html=True)
...
def run_streamlit():
    ...
    decomposition_fig, model_results_diagnostics, forecast_overlay_fig = plot_model_figures(model_results, cleaned_dataframe, alpha_level, hyperparameter_combo)

    model_evaluation(cleaned_dataframe, hyperparameter_combo)

    forecast_fig = forecast_vizuals(cleaned_dataframe, alpha_level, hyperparameter_combo, forecast_start_date, forecast_end_date)
    ...
    if st.button('Download Forecast Files'):
        ...
        st.write(f'''Downloaded all files:
        ...
        merged_forecast_figs.to_csv(f"{merged_table_title.lower()}_({forecast_start_date} - {forecast_end_date})_forecast_table.csv", index = False)
        decomposition_fig.savefig(f'{merged_table_title.lower()}_({forecast_start_date} - {forecast_end_date})_data_decomposition.pdf')
        model_results_diagnostics.savefig(f'{merged_table_title.lower()}_({forecast_start_date} - {forecast_end_date})_data_diagnostics.pdf')
        forecast_overlay_fig.savefig(f'{merged_table_title.lower()}_({forecast_start_date} - {forecast_end_date})_forecast_overlay.pdf')
        forecast_fig.savefig(f'{merged_table_title.lower()}_({forecast_start_date} - {forecast_end_date})_forecast_graph.pdf')
